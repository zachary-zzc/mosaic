...
% ============================================================================
\begin{abstract}
Large language model (LLM) agents augmented with persistent memory can recall
past interactions but frequently store contradictory or erroneous information
without detection, leading to compounding errors across the memory pipeline.
We present \textbf{MOSAIC} (\textbf{M}emory-\textbf{O}rganized
\textbf{S}tructured \textbf{A}gent for \textbf{I}nformation
\textbf{C}ollection), a structured long-term memory framework that organizes
memories as a typed, conflict-aware graph with active validation during
ingestion. MOSAIC introduces three key capabilities beyond existing memory
systems: (1)~entity-typed graph storage with semantic classification that
preserves relational structure across events, personas, and relationships;
(2)~conflict detection at save time that cross-references new information
against existing memories, issuing warnings and triggering updates or
deletions for contradictory entries; and (3)~hash-accelerated retrieval
achieving 45$\times$ faster memory lookup than the strongest baseline with
negligible accuracy loss. We evaluate MOSAIC on three benchmarks: LoCoMo
(long-conversation memory QA), HaluMem (memory extraction, updating, and
question answering with hallucination detection), and a novel error
compounding test using manually injected contradictions in hypertension
clinical guidelines. MOSAIC achieves 79.7\% overall accuracy on LoCoMo
(+17.5~pp over the best baseline), 0.89 F1 on memory extraction, and detects
66\% of injected factual conflicts---4.7$\times$ higher than the best
baseline (14\%). These results demonstrate that structured, conflict-aware
memory organization substantially outperforms flat memory stores for
long-term LLM agent memory.
\end{abstract}
...
Mem0~\citep{mem0_2024}, A-Mem~\citep{amem_2024}, and
MemOS~\citep{memos_2024} have demonstrated impressive retention of user
...
harmful prescribing~\citep{sharma2024nature_medicine}. In legal and financial
...
coverage~\citep{xi2023rise, wang2024survey_agents}. Current agent
...
programming~\citep{bellman1957dynamic}, which decomposes a global
...
Mem0~\citep{mem0_2024} provides multi-level persistent memory with adaptive
personalization across user, session, and agent levels.
A-Mem~\citep{amem_2024} implements adaptive memory with automatic storage,
retrieval, and summarization, dynamically adjusting memory granularity.
MemOS~\citep{memos_2024} unifies memory operations through an
...
Zep~\citep{zep_2024} provides production-grade memory with temporal awareness
...
A comprehensive survey by \citet{zhang2024survey_memory} categorizes memory
...
MOSAIC's design choices. ReAct~\citep{yao2023react} interleaves reasoning
...
information management~\citep{xi2023rise, wang2024survey_agents}.

Tool-augmented approaches~\citep{schick2024toolformer} extend agents with
...
structure but does not perform conflict checking. GraphRAG~\citep{graphrag2024}
...
LoCoMo~\citep{locomo_2024} evaluates long-conversation memory through
multi-type QA spanning single-hop, multi-hop, temporal, and open-domain
questions. HaluMem~\citep{halumem_2024} introduces hallucination-aware
evaluation of the memory pipeline, separately assessing extraction, updating,
and question answering stages. LLM-as-Judge
approaches~\citep{zheng2024judging} supplement reference-based evaluation. Our
...
studied~\citep{huang2023survey_hallucination, ji2023hallucination}. Fact
verification systems~\citep{thorne2018fact} and fine-grained factual
...
algorithm~\citep{traag2019leiden} to $G_A$ at initialization.
...
\begin{itemize}[noitemsep,topsep=3pt]
  \item \textbf{LLM path}: Full LLM-based semantic classification achieving
    ...
    tokens).
  \item \textbf{Hash path}: Locality-sensitive
    hashing~\citep{indyk1998lsh} for near-instantaneous classification at
    0.1s per query (save time: $\sim$1.5h per 100k tokens), with only
    1.5~pp accuracy reduction.
\end{itemize}

An additional \emph{no-class} variant bypasses classification entirely
(0.015s per query, $\sim$1h save time) while maintaining competitive
accuracy, suggesting that the graph structure itself captures sufficient
relational information even without explicit type labels.

\subsection{Community-Aware Retrieval}
\label{sec:retrieval}

For memory retrieval, MOSAIC leverages the graph structure through
community detection (Leiden algorithm~\citep{traag2019leiden}) over the entity
...
search cannot support~\citep{lewis2020rag, gao2024rag_survey}.
...
submodular optimization~\citep{nemhauser1978submodular}.
...
\paragraph{LoCoMo.}
The LoCoMo benchmark~\citep{locomo_2024} evaluates long-conversation memory
...
evaluation protocol~\citep{zheng2024judging} to assess answer correctness.

\paragraph{HaluMem.}
The HaluMem benchmark~\citep{halumem_2024} evaluates three stages of the
...
a comprehensive assessment of the full memory pipeline.
...
\begin{itemize}[noitemsep,topsep=3pt]
  \item \textbf{A-Mem}~\citep{amem_2024}: Adaptive memory with automatic
  ...
  \item \textbf{Zep}~\citep{zep_2024}: Production memory system with
    temporal awareness and entity extraction.
  \item \textbf{OpenAI Memory}: Built-in conversational memory from
    OpenAI~\citep{achiam2023gpt4}.
  \item \textbf{Mem0}~\citep{mem0_2024}: Multi-level memory (user, session,
...
For HaluMem, additional baselines include MemOS~\citep{memos_2024},
...
All experiments use Qwen-2.5-32B~\citep{qwen2024, yang2024qwen2} as the base
LLM for both MOSAIC and all baselines, ensuring fair comparison. Entity
embeddings use all-MiniLM-L6-v2~\citep{reimers2019sentence} with $d = 384$
dimensions. The association graph threshold is $\theta = 0.4$, and the
confidence gating threshold is $\rho_{\min} = 0.6$. Scoring weights are
$\alpha = 0.5$, $\beta = 0.3$, $\gamma = 0.2$. Memory retrieval uses
top-$m = 5$ neighbors.
...
from retrieved text fragments~\citep{liu2024lost_middle}.
...
\midrule
Sense+LLM & Sense+LLM & 4.5h & 6.8s/q & 79.68 \\
Sense+LLM & Hash & 4.5h & 0.1s/q & 79.61 \\
Sense+Hash & Hash & 1.5h & 0.1s/q & 78.18 \\
No class & No class & 1.0h & 0.015s/q & 79.81 \\
\bottomrule
\end{tabular}
\end{table}

The hash-accelerated query path achieves 79.61\% accuracy (vs 79.68\% with
full LLM classification), a negligible 0.07~pp difference with a
\textbf{68$\times$ speedup} (0.1s vs 6.8s per query). The no-class variant
achieves the highest overall accuracy (79.81\%) while being the fastest,
suggesting that explicit semantic classification may be unnecessary when the
graph structure already encodes sufficient relational information. This
...
Table~\ref{tab:halumem} presents results on the HaluMem benchmark, which
evaluates three stages of the memory pipeline independently. MOSAIC achieves
state-of-the-art performance across all three stages.
...
Figure~\ref{fig:efficiency} presents execution time comparisons on the
HaluMem benchmark. MOSAIC achieves a memory retrieval time of
\textbf{0.46~minutes} across the entire benchmark---a \textbf{45$\times$
speedup} over the next-fastest system (MemOS, 20.52~min). This dramatic
speedup is attributable to the hash-accelerated retrieval path, which
replaces expensive LLM-based classification with locality-sensitive hashing
for entity type matching and community-based indexing for neighborhood
retrieval.

Dialogue addition time (346.57~min) is moderate, comparable to
Supermemory (273.21~min) and Memobase (293.30~min), and substantially
faster than Mem0 (2,768.14~min) and Mem0-Graph (2,840.07~min). The
addition time includes the overhead of conflict detection (approximately
15\% of total addition time), which is a worthwhile investment given the
substantial improvement in downstream accuracy.

The hash-accelerated retrieval path (Table~\ref{tab:ablation}) further
demonstrates the speed--accuracy trade-off: switching from LLM-based
classification (6.8s/query) to hash-based classification (0.1s/query)
...
\begin{figure}[!t]
  ...
  \end{subfigure}
  \caption{\textbf{Computational efficiency on HaluMem.} (a)~Total execution
  time showing dialogue addition and retrieval components. MOSAIC has
  moderate addition time but near-instantaneous retrieval. (b)~Memory
  retrieval time: MOSAIC achieves 0.46~min, 45$\times$ faster than MemOS
  (20.52~min).}
