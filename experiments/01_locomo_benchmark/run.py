...
# -*- coding: utf-8 -*-
"""
Experiment 1 — LoCoMo 主评测：对 mosaic/src 下已就绪的 LoCoMo 图跑 QA，合并参考基线，输出 tab_locomo.tex 与 JSON。

与 experiments/README.md「Experiment 1」一致；全量 B1–B8 基线需在统一协议下分别接入各方法后汇总。
"""
import os
import sys
import argparse
...
def aggregate_summaries(results_dir: str):
    ...
    for name in os.listdir(results_dir):
        # 仅汇总 run.py 产生的 qa_*_summary.json，避免与 qa_eval.py 的 mosaic_* 混计
...
def build_full_locomo_table(mosaic_metrics=None):
    """完整表：参考值 + 可选 MOSAIC 实测。"""
    table = {}
    for method, row in ref.LOCOMO_TABLE.items():
        table[method] = dict(row)
    if mosaic_metrics and mosaic_metrics.get("overall_total", 0) > 0:
        table["MOSAIC"] = {
            "overall": mosaic_metrics["overall_accuracy_pct"],
            "single_hop": mosaic_metrics["by_category_pct"].get("Single-hop", 0),
            "multi_hop": mosaic_metrics["by_category_pct"].get("Multi-hop", 0),
            "temporal": mosaic_metrics["by_category_pct"].get("Temporal", 0),
        }
    return table
...
def main():
    ...
    parser.add_argument("--skip-run", action="store_true", help="仅汇总已有结果并导出，不跑 QA")
    args = parser.parse_args()

    results_dir = ensure_results_dir("01_locomo_benchmark")
    ...
    if not args.skip_run and pairs:
        ...
        mosaic_metrics = aggregate_summaries(results_dir)
    elif args.skip_run and os.path.isdir(results_dir):
        mosaic_metrics = aggregate_summaries(results_dir)

    full_table = build_full_locomo_table(mosaic_metrics)
    save_json(
        os.path.join(results_dir, "locomo_metrics.json"),
        {"mosaic_run": mosaic_metrics, "full_table": full_table},
    )
    ...
    save_json(os.path.join(results_dir, "locomo_table_mosaic.json"), full_table)
    return 0
